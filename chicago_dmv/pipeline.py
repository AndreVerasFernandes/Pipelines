...
def run_pipeline():
    ...
    # Step 3: Load data
    load.load_data(
        df=crashes_transformed_df, 
        table_name=config_data['crash_table_PSQL']
    )
    
    load.load_data(
        df=vehicle_transformed_df, 
        table_name=config_data['vehicle_table_PSQL']
    )
    
    load.load_data(
        df=people_transformed_df, 
        table_name=config_data['person_table_PSQL']
    )
